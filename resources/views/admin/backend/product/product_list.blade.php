...
<div class="card-body">
    ...
    <tr>
        ...
        <td>
            ...
            @if($expiredDate)
                <div>{{ $expiredDate->format('M d, Y') }}</div>
                @if($expiredDate->isPast())
                    <span class="badge bg-danger">Expired {{ abs($daysDiff) }} days ago</span>
                @else
                    <span class="badge bg-success">{{ $daysDiff }} days left</span>
                @endif
            @else
                <div class="text-muted">No Expiry Date</div>
